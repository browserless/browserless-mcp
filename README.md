...
| `BROWSERLESS_API_SERVER`     | Self-hosted account tools | `https://api.browserless.io`             | Account API host — backs `browserless_account`, `_usage`, `_sessions` and `_logs`. Set this for self-hosted runtimes; `browserless_logs` fails closed before sending the token when it is omitted. A different host from `BROWSERLESS_API_URL`, which is a browser runtime |
