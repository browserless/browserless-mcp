...
5. **Re-snapshot** only if the page changed (click, goto, submit, navigation)
...
- Your snapshot is STALE after: click, goto, select (may trigger navigation), submit, any navigation
...
2. Classify each action:
   - **Safe to batch** (no page change): type, hover, scroll, evaluate, select (non-navigating)
   - **Page-changing** (triggers reload/navigation): click (buttons/links), goto, submit
...
- **scroll** { selector?, direction? } — scroll page or element
