...
description: >-
  ...
  API with no auth or anti-bot stealth.
...
alternative_methods:
  - method: browser
    rationale: >-
      ...
      with no anti-bot), but costs a browserless_agent session and 2 page-loads
      vs. the API path's 2 same-origin HTTP calls. Use only if the MediaWiki API
      is unreachable — a multi-9s availability service.
  - method: api
    rationale: >-
      Pageviews API
...
The recommended path is the **MediaWiki public API** — no auth, no cookies, no anti-bot stealth, no browser session needed beyond a single `browserless_function` per surface (page in the origin, then a same-origin `fetch`). Two requests to `https://en.wikipedia.org/w/api.php` are sufficient. Browser navigation is a fallback that costs a full `browserless_agent` session and gives the same data.
...
Through Browserless — use `browserless_function`. Per the runtime note, code runs in a browser page context and a bare `fetch()` has no network egress until the page is navigated, so `page.goto` the Wikipedia origin first, then run a **same-origin** `fetch` to `/w/api.php` inside `page.evaluate`. Project the wikitext string inside the eval so you never ship the whole envelope back:

```js
export default async ({ page }) => {
  const portal = `${new Date().getUTCFullYear()}_${new Date().toLocaleString('en-US', { month: 'long', timeZone: 'UTC' })}_${new Date().getUTCDate()}`;
  await page.goto('https://en.wikipedia.org/', {
    waitUntil: 'load',
    timeout: 45000,
  });
  const wikitext = await page.evaluate(async (p) => {
    const url = `/w/api.php?action=parse&page=Portal:Current_events/${p}&format=json&prop=wikitext`;
    const r = await fetch(url);
    const j = await r.json();
    return j.parse && j.parse.wikitext ? j.parse.wikitext['*'] : '';
  }, portal);
  return { data: wikitext, type: 'application/json' };
};
```

`fetch` is same-origin (`en.wikipedia.org` → `/w/api.php`), so it resolves cleanly once the page is on the origin. The page wikitext lands in `parse.wikitext['*']`. The body shape is:
...
If "trending" should include _what readers are clicking on_ (not just what editors curated), add a third call to the Wikimedia pageviews API:

```
GET https://wikimedia.org/api/rest_v1/metrics/pageviews/top/en.wikipedia/all-access/{YYYY}/{MM}/{DD}
```

This returns the top 1000 most-viewed pages on English Wikipedia for the given day. Filter out housekeeping pages (`Main_Page`, `Special:Search`, `Wikipedia:*`) and you get a real-time view of which articles are surging — often a leading indicator that an event is breaking before ITN editors curate it. Returns within a few hours' lag from real-time. No auth. Note this endpoint lives on `wikimedia.org`, a **different origin** from `en.wikipedia.org`: run it as its own `browserless_function` that `page.goto('https://wikimedia.org/')` first, then `page.evaluate` a same-origin `fetch` of the `/api/rest_v1/...` path (the REST API CORS-permits, but same-origin navigation is the reliable path).
...
If the MediaWiki API is unreachable (very rare — Wikipedia's API has multi-9s uptime and serves billions of requests/day), fall back to navigating the rendered pages directly with a single `browserless_agent` call. Both page loads go in **one** `commands` array so they share the one session in the fewest round-trips — there is no session-release step (nothing to release). The session persists across separate calls too, keyed by `proxy`/`profile`. Use `text` to pull the readable rendering of each page's body:
...
```json
{
  "commands": [
    ...
    { "method": "text", "params": { "selector": "#mp-itn" } },
    {
      "method": "goto",
      "params": {
        "url": "https://en.wikipedia.org/wiki/Portal:Current_events/2026_May_18",
        "waitUntil": "load",
        "timeout": 45000
      }
    },
    {
      "method": "text",
      "params": { "selector": ".current-events-content, #mw-content-text" }
    }
  ]
}
```

Compute the portal subpage title (`Portal:Current_events/{YYYY}_{Month}_{D}`) the same way as the API path and substitute it into the second `goto` URL. The Main Page ITN box lives under `#mp-itn` (the "In the news" column); the portal body is under `.current-events-content` / `#mw-content-text`. `text` returns the readable text of the matched region with category headings and bullets preserved — easier to parse than raw HTML but slower and costlier than the API path (one browser session + 2 page loads vs. 2 same-origin HTTP calls). Confirm the selectors via a `snapshot` command if `text` comes back empty.

No stealth session or proxy is required for either path. Wikipedia explicitly welcomes bots that follow the [API etiquette guidelines](https://www.mediawiki.org/wiki/API:Etiquette) — set a descriptive `User-Agent` if making many requests, and keep concurrency low.
