...
The Craigslist web UI is a thin client over a public JSON API at `https://sapi.craigslist.org` — no auth, no cookies, no session state, no anti-bot stealth required. Send a `Referer` header matching the target city subdomain; if your outbound IP is in a different region than the target city, add `postal=<zip>&search_distance=<mi>` to the query — the API geo-scopes by IP only when no `postal` is supplied (see the gotcha below). **A residential proxy is not required.** To call the JSON API, use `browserless_function`: `page.goto('https://sapi.craigslist.org/')` first (a bare `fetch` has no egress until the page navigates to that origin), then `page.evaluate(async () => (await fetch('/web/v8/postings/search/full?...')).json())` — same-origin, so no CORS issue. Project/summarize inside the eval (the text return is capped ~200k chars). Lead with the API path; the browser path works as a fallback but pays a ~100× cost premium because the search page is fully JS-rendered (a `snapshot` returns 0 a11y refs and harvesting per-listing URLs costs ~3 turns each).
...
When the API is unreachable or geo-locked away from the target city (rare — `postal=<zip>` almost always resolves it), use a `browserless_agent` call: `goto` `https://{city}.craigslist.org/search/{cat}?query={q}&sort=date` directly (bypassing the bare-domain geo-redirect) with `waitUntil: "load"`, then capture the rendered DOM with an `html` command (`{ "method": "html", "params": { "selector": "body" } }`) or fold the parse into an `evaluate`. Split per-listing chunks by the regex `<div data-pid="(\d+)" class="cl-search-result`. Within each chunk, extract:
...
Skip `snapshot`/`click` on `/search/` — snapshot returns 0 refs and click-through costs ~3 turns per listing. Stable across `cta` and `apa` in prior validation.
...
- **API geolocates by request IP — `postal=<zip>&search_distance=<mi>` overrides it**: No auth, no cookies, no anti-bot — but if no `postal` is supplied, the API scopes results to the city corresponding to the request's source IP, not the `Referer` header (e.g. a NY query from an SF IP silently returns `{"1": {"name": "sfbay"}}` results). Adding `postal=<zip>` for any ZIP in the target metro plus `search_distance=<mi>` forces the result set to that region. Re-verified 2026-05-19 with direct `browserless_function` fetches returning correct NYC apartments (`postal=10001&search_distance=10`, 860 results, `data.areas` shows `newyork/newjersey/longisland/hudsonvalley` cluster) and SF Bay bicycles (`postal=94103&search_distance=25`, 5,635 results, `data.areas: {"1": "sfbay"}`). **A residential proxy is not required and is actively counterproductive** — a proxied fetch _without_ `postal` is also geo-locked to the proxy's exit-IP region, and adding `postal` to a direct fetch is faster than routing through a proxy. Always verify scope via `data.areas` in the response.
- **Snapshot returns 0 refs on `/search/`**: The search page is fully JS-rendered (React). Don't use `snapshot`/`click` to enumerate listings — fall back to an `html` command + regex per the Browser fallback section.
...
- **Don't waste time on stealth fingerprinting** — the API has no anti-bot today (verified 2026-05-19 via a direct unproxied `browserless_function` fetch returning 200 + 134 KB JSON on the first try with no Referer). A stealth session on residential proxies does not improve success rate and actively slows the path.
