...
The optimal path is direct HTTP. The endpoint is public, unauthenticated, CORS-enabled, and not anti-bot protected. There is **no benefit to scripted browsing of the WDQS web UI** — every shape of the SPARQL contract is reachable from `curl`. Honesty bar: no proxies, no stealth session, no stealth flag needed. The browser fallback below exists only for the contingency where direct HTTP egress to `query.wikidata.org` is blocked at the network layer.
...
### Browserless fallback (last resort)

The curl / HTTP examples above are canonical — the endpoint is public, unauthenticated, and CORS-enabled, so run them from any client. The fallbacks below matter only when a client can't reach `query.wikidata.org` directly (restricted egress) but a Browserless page can.

**Fallback 1 — same-origin fetch via `browserless_function`.** `browserless_function` runs in a browser page context, not Node, and a bare `fetch(url)` has no network egress until the page is navigated. So navigate the page to the Wikidata origin first, then run the fetch in-page (same-origin — WDQS is CORS-enabled). No proxy, no stealth — the endpoint is not anti-bot protected. Decode/resolve per Path A steps 4–5 **inside** the eval; the text return is capped (~200 KB), so project or summarize large result sets in-page rather than shipping raw bindings.

```js
export default async function ({ page }) {
  const query =
    'SELECT ?item ?itemLabel WHERE { ?item wdt:P31 wd:Q5 . SERVICE wikibase:label { bd:serviceParam wikibase:language "en" } } LIMIT 100';
  await page.goto('https://query.wikidata.org/', {
    waitUntil: 'load',
    timeout: 45000,
  });
  const out = await page.evaluate(async (q) => {
    ...
    // flatten bindings to { var: value } here — never return the raw envelope for huge result sets
    return {
      httpStatus: res.status,
      vars: body.head.vars,
      rows: body.results.bindings,
    };
  }, query);
...
`Special:EntityData` (Path B) is same-origin to `www.wikidata.org`, so the identical pattern works — `page.goto('https://www.wikidata.org/')` then in-page `fetch('https://www.wikidata.org/wiki/Special:EntityData/Q42.json')`.

**Fallback 2 — drive the WDQS web UI via `browserless_agent`.** Only if even the in-page fetch is blocked. WDQS auto-loads a query from the URL fragment; keep navigate → run → read-results in ONE `browserless_agent` call's `commands` array (fewer round-trips; no session-release step — there's nothing to release, and cookies/state persist across the steps). The session persists across separate calls too, keyed by `proxy`/`profile`, so a follow-up call with the same config reconnects to the same state. No proxy, no stealth — WDQS is bare-friendly.
  ...
  "commands": [
    {
      "method": "goto",
      "params": {
        "url": "https://query.wikidata.org/#<URL-encoded SPARQL>",
        "waitUntil": "load",
        "timeout": 45000
      }
    },
    { "method": "click", "params": { "selector": "button#execute-button" } },
    {
      "method": "waitForSelector",
      "params": { "selector": ".result-table, .th-inner", "timeout": 30000 }
    },
    {
      "method": "evaluate",
      "params": {
        "content": "(()=>{ const rows=[...document.querySelectorAll('.result-table tbody tr')].map(tr=>[...tr.querySelectorAll('td')].map(td=>td.innerText.trim())); return JSON.stringify(rows); })()"
      }
    }
  ]
}
...
Confirm the run-button and result-pane selectors with a `snapshot` command if they miss. This path is ~50× slower than the direct HTTP paths and only worth it as a last-mile fallback. Don't lead with it.
  ...
  "rows": [
    {
      "item": {
        ...
        "url": "https://www.wikidata.org/entity/Q23"
      },
...
```json
{
  ...
  "claims": {
    "P31": [{ "value": { "qid": "Q5", "label": "human" }, "rank": "normal" }],
    "P21": [/* ... */]
  },
  "sitelinks": {
