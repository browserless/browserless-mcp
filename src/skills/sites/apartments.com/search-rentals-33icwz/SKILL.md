...
alternative_methods:
  - method: fetch
    rationale: >-
      ...
      shortcut; a stealth + residential-proxy browser session (browserless_agent
      with proxy: residential) is the only working path.
verified: true
proxies: true
---
...
challenge. You must drive a stealth + residential-proxy browser session via
`browserless_agent`. Deep-link the search URL directly (no need to type into the search
box), then extract listings with one `evaluate` reading the result placards.

Run the whole flow — navigate → wait → extract — inside **one** `browserless_agent` call
so the session (cookies, Akamai clearance) persists across the commands in its `commands`
array. Batching this way saves round-trips and avoids accidentally dropping the session
config; the session itself persists across calls, keyed by `proxy`, so a later call with
the same `proxy` reconnects to it while dropping or changing `proxy` lands you in a
different, blank session.

### 1. Use a stealth + residential-proxy session (MANDATORY)

Set a residential proxy on the `browserless_agent` call:

```
proxy: { proxy: "residential" }   // optionally proxyCountry: "us"
```

Stealth is on by default. A proxy-less session is denied immediately on the search page.
Even a fully-stealthed + proxied session is blocked intermittently — see Gotchas; budget
for fresh-call retries (issue a new `browserless_agent` call to rotate the
session / proxy IP).
...
```json
{ "method": "goto", "params": { "url": "https://www.apartments.com/chicago-il/under-2500/", "waitUntil": "load", "timeout": 45000 } },
{ "method": "waitForTimeout", "params": { "time": 3000 } }
```

Read the page title with an `evaluate` returning `document.title` — expect "Apartments
for Rent ... in Chicago IL | Apartments.com".

If the title returns **"Access Denied"**, the session got challenge-blocked. Issue a
`{ "method": "reload", "params": { "waitUntil": "load" } }` and wait ~6s — it frequently
renders on the 2nd try in the same session. If it's still denied after 2 reloads, abandon
this call and start a fresh `browserless_agent` call. Do NOT loop reloading the same
session many times (that wastes turns and never recovers).

Do **not** use the `snapshot` command — the page has thousands of nodes; the snapshot is
enormous and unnecessary.

### 3. Extract all placards with one `evaluate`

The search results are `article.placard` elements (~40 per page). Run this as the
`evaluate` command's `content` (it returns a compact JSON string — proven across multiple
runs; the return value comes back under `.value`):
...
(() => {
  ...
  const cnt = document.querySelector(
    '.searchResults, .resultSummary, [class*=resultsCount]',
  );
  const m =
    (cnt ? cnt.textContent : document.title || '').match(
      /([\d,]+)\s+Rentals/i,
  ...
  for (const card of document.querySelectorAll('article.placard')) {
    const name = (card.querySelector('.js-placardTitle, .title') || {})
      .textContent;
    ...
    for (const box of card.querySelectorAll('.bedRentBox')) {
      const b = box.querySelector('.bedTextBox'),
        p = box.querySelector('.priceTextBox');
    ...
    const bedRange =
      beds.length === 1
        ? beds[0]
        : beds.length > 1
          ? beds[0] + ' - ' + beds[beds.length - 1]
          : null;
    const phone = (card.querySelector('.phone-link span, a.phone-link') || {})
      .textContent;
    if (name || url)
      out.listings.push({
...
Parse the returned `.value` string into your output object. The eval yields ~40 clean
...
budget. The evaluate's returned value is the source of truth; surface a representative
slice (e.g. first 12) inline and note that `result_count_on_page` reflects the full page.
...
- **No API / HTTP-fetch path — confirmed blocked.** Direct `GET` of search URLs returns
  `403 AkamaiGHost` even with residential proxies (verified via a raw HTTP fetch both
  with and without a residential proxy → 403). Don't waste time hunting for a JSON
  endpoint; you must use a full stealth browser.
- **Stealth + a residential proxy are both mandatory.** A bare session is denied on the
  search page. Set `proxy: { proxy: "residential" }` on the `browserless_agent` call
...
Denied</title>`, body referencing `errors.edgesuite.net`). Behavior observed during
  testing: sometimes the first navigation is blocked but a single `reload` command
  recovers it in the same session (run-002); other times _every_ fresh session is blocked
  for a sustained stretch (4+ consecutive fresh sessions denied during one window).
  **Treat success as probabilistic**: retry with a fresh stealth + residential-proxy
  `browserless_agent` call, up to ~3–4 attempts, with a `reload` fallback per session.
  The block rate appears to climb after repeated hits from the same proxy pool, so space
  requests out.
...
- **Don't dump page text.** A `text` command on `body` / `#placardContainer` returns
  ~300KB and blows the inner agent's max output tokens (truncated run-001 and run-003).
  Always extract with the targeted `evaluate` above.
- **Don't use the `snapshot` command** on the results page — thousands of a11y nodes; not
...
```json
{
  ...
  "listings": [],
  "error_reasoning": "Akamai 'Access Denied' on every attempt across 4 fresh stealth + residential-proxy sessions; proxy pool IP reputation appears flagged for this window."
}
```
