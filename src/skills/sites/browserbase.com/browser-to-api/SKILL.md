...
### 1. Capture with `browser-trace` (and optionally bodies via `network capture on`)
...
# attach your browser driver to $TARGET and navigate it to about:blank
network capture on                                    # capture request/response bodies
# navigate the driver to the page(s) you want covered, e.g. https://example.com
# ...drive whatever flows you want covered...

# Snapshot the bodies dir BEFORE turning capture off (the temp dir is shared
# per-session, so subsequent `network capture on` runs would mix your bodies
# with whatever a future capture writes if you skip this step).
cp -r "$(network capture path | jq -r .path)" .o11y/my-site/cdp/network/bodies/
network capture off
...
`network capture on` is **optional but strongly recommended** — without it, the spec has no response-body schemas (the CDP firehose used by a CDP connection does not embed bodies). With it, both request bodies (already captured by CDP) _and_ response bodies are joined into the trace by CDP `requestId`.
...
`discover.mjs` auto-detects `<run>/cdp/network/bodies/`. To use a body capture from elsewhere (e.g. didn't snapshot, want the live network capture dir), pass `--bodies <path>` explicitly.
...
| `--bodies <path>`             | no       | network capture capture dir to join into the trace (auto-detected from `<run>/cdp/network/bodies/` when present) |
...
## What you get from a CDP connection and network capture

Two complementary capture sources:

| Source                                     | Provides                                                                                   | Limitation                                                                                                               |
| ------------------------------------------ | ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------ |
| a CDP connection (used by `browser-trace`) | request method/URL/headers/`postData`, response status/headers/mimeType, full event timing | **Does not embed response bodies.** Bodies must be pulled with `Network.getResponseBody`, which the firehose doesn't do. |
| `network capture on` (separate command)    | request bodies AND response bodies on disk, keyed by CDP `requestId`                       | Capture dir is shared per `browse` session; snapshot before another `network capture on` overwrites it.                  |

`discover.mjs` will pull bodies from a network capture dir if you pass `--bodies <path>` (or stash them under `<run>/cdp/network/bodies/`, which is auto-detected). The matching is by `requestId` — network capture writes that into each `request.json` as `id`, and we join directly.
...
5. **Pair with `network capture on`** when response-body schemas matter. The CDP firehose alone has request bodies but not response bodies.
