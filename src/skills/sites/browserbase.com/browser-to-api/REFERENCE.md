...
- Pairs by `requestId`. Drops `OPTIONS` (CORS preflight) and pure redirects (status 3xx with `Location` and no body — recorded as metadata on the _next_ request in the chain when the requestId carries forward, otherwise dropped).
...
| `--origins <list>`            | none             | Comma-separated. If set, anything _not_ matching is dropped before include/exclude                               |
