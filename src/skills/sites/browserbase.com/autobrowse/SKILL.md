...
/autobrowse --task google-flights --iterations 10
/autobrowse --task google-flights browser-trace mode
...
`browser-trace mode` (default off, remote-only): pairs each iteration with the sibling `browser-trace` skill — wraps the inner agent in a CDP capture for per-page network/console/page-lifecycle evidence. Implies ` remote`; errors if combined with ` local`. Requires the sibling `browser-trace` skill present at `${CLAUDE_SKILL_DIR}/../browser-trace/`, and the `BROWSERBASE_API_KEY` env var.
...
- ` local|remote` → browser environment (default: local; use remote for bot-protected sites)
- `browser-trace mode` → opt in to the browser-trace integration (default off). Implies ` remote`. If ` local browser-trace mode` are both passed explicitly, error with: `browser-trace requires Browserbase; drop or drop browser-trace mode.`
...
> "You are running the autobrowse skill for task `<name>`. Workspace: `<absolute-path-to-workspace>` (e.g. `/path/to/project/autobrowse`). Run `<N>` iterations of: evaluate → read trace → improve strategy.md → repeat. Use ` <env>`. Pass ` <workspace>` to every evaluate.mjs invocation. If the parent invocation used `browser-trace mode`, you MUST use the traced-path block of the SKILL.md loop for every iteration (pre-create session, attach bb-capture, pass the session connect-url to evaluate.mjs, stop+bisect, release) — do not fall back to the default single-command path. Follow the autobrowse loop instructions exactly.
>
...
**Default path (no `browser-trace mode`)** — single command, no orchestration:
...
**Traced path (`browser-trace mode`, remote only)** — the outer harness pre-creates a Browserbase session, attaches `bb-capture` as a passive observer, and passes the session's `connectUrl` to `evaluate.mjs` so every inner `browse` call uses `$connectUrl (the canonical browser-trace pattern that gives observers full Network/Console events). Run this block once per iteration with`$N` set to the 1-indexed iteration number:
...
echo "ERROR: browser-trace mode requires the browser-trace skill at $BT_DIR." >&2
...
# a. SESSION SETUP — create a kept-alive stealth + residential-proxy session via

# the Browserbase platform API, then read its id and connectUrl from the JSON.

sid=$(create_session_via_platform_api \
 | node -e "let s='';process.stdin.on('data',c=>s+=c).on('end',()=>process.stdout.write(JSON.parse(s).id))")
connect_url=$(get_session_via_platform_api "$sid" \
...
# c. RUN AUTOBROWSE — the session connectUrl tells evaluate.mjs to inject

# / into every inner browse call. The inner agent never sees a

# remote-session flag; pass "$connect_url" to evaluate.mjs (via its connect-url

# option / env) so it attaches to the pre-created session.
...
# e. RELEASE — request release of the session via the Browserbase platform API

release_session_via_platform_api "$sid"
`
...
Under `browser-trace mode`, the hypothesis must cite a **specific event from `unified-events.jsonl`** (line number or timestamp) — or name the drill-down file if you had to descend into one. This keeps updates evidence-grounded rather than vibes-driven. A hypothesis based only on the agent's commands might say "the click didn't work"; grounded in the unified stream, it can say "line 47 of unified-events.jsonl: a page navigation was followed by `Network.responseReceived` status 403 on `/api/checkout` — switch to a stealth + residential-proxy session."
...
- "Use a direct `fill` on `#field_3` rather than `type` — this field clears on focus"
- "The page shows a spinner at turn 8 — add a 2000 ms wait before the snapshot"
- (with `browser-trace mode`) "At line 47 of unified-events.jsonl, 3 consecutive `Network.responseReceived` events on `/api/availability` returned 403 right after the page navigation — the site is fingerprinting; the next iter needs a stealth + residential-proxy session."
...
The inner agent drives the browser through the `browserless_agent` command set. Key commands for this task:

- `{ "method": "goto", "params": { "url": "<url>", "waitUntil": "load", "timeout": 45000 } }` — navigate to a page (never use networkidle0/2; it hangs on SPAs)
- `{ "method": "waitForTimeout", "params": { "time": <ms> } }` — wait a fixed amount of time for spinners or animations
- `{ "method": "waitForSelector", "params": { "selector": "<selector>", "timeout": 10000 } }` — wait for an element to become visible
- `{ "method": "text", "params": { "selector": "body" } }` — extract visible text (preferred for content extraction)
- `{ "method": "evaluate", "params": { "content": "(()=>{ ... })()" } }` — parse in-page and return a compact `JSON.stringify` projection (result comes back under `.value`)
- `{ "method": "snapshot" }` — get the accessibility tree
- `{ "method": "click", "params": { "selector": "<selector>" } }` — click an element
- Chain the whole flow (nav → wait → extract → paginate) inside ONE call's `commands` array — the session persists across separate calls (keyed by `proxy`/`profile`), so this is a convenience that saves round-trips and avoids accidentally dropping the session config, not a lifetime requirement.

**Keep infrastructure concerns out of SKILL.md.** Skills must work in isolation — don't hardcode session names, proxies, or connection details.
...
- **Don't release before bisecting** — under `browser-trace mode`, the order at the end of each iteration is non-negotiable: `stop-capture` → `bisect-cdp` → session release (via the Browserbase platform API). Bisect depends on the session still existing when the trace stops.
