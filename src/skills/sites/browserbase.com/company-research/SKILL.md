...
license: MIT
compatibility: Requires the Browserless MCP server (endpoint + bearer token). Web search uses the `browserless_search` tool; page extraction uses `browserless_agent` (goto + text/evaluate) or `browserless_function`.
...
**Required**: the Browserless MCP server configured (endpoint + bearer token) so the `browserless_search` and `browserless_agent`/`browserless_function` tools are available.

**First-run setup**: On the first run you'll be prompted to approve the Browserless MCP tools (`browserless_search`, `browserless_agent`) and the local helper Bash commands (`node`, `cat`, `mkdir`, `sed`, etc.). Approve each to auto-approve for the session. To permanently approve the local shell helpers, add these to your `~/.claude/settings.json` under `permissions.allow`:

```json
"Bash(bunx:*)", "Bash(bun:*)", "Bash(node:*)",
...
- All web searches: use the `browserless_search` MCP tool. NEVER use WebSearch.
- All page content extraction: use `node {SKILL_DIR}/scripts/extract_page.mjs "<url>"`. This wrapper pulls the page through Browserless — a lightweight fetch first, then falling back to a full `browserless_agent` render (`goto` + `text`/`evaluate`) when the fetch fails or returns thin JS-rendered content — and parses title + meta tags + visible body text. NEVER hand-roll a raw HTML dump + `sed` pipeline — it strips meta tags and doesn't parse the structured envelope; let `browserless_agent`'s `text`/`evaluate` parse in-page instead. NEVER use WebFetch. (The bundled `extract_page.mjs` must be pointed at your Browserless endpoint; it wraps the same two tools.)
...
3. **Run a full deep research on the user's company** using the Plan→Research→Synthesize pattern.
   ...
   **Key research steps:**
   - Search: `browserless_search` for `"{company name}"` (request ~10 results)
   ...
   - **Discover site pages via sitemap** (do NOT hardcode paths like `/about` or `/customers`):
     1. Pull `"{company website}/sitemap.xml"` — the sitemap is small XML, so a plain `browserless_agent` `goto` + `text` (or a `browserless_function` fetch) is fine
     ...
     4. Pick 3-5 most relevant URLs and extract with `extract_page.mjs` (NOT a raw HTML dump)
...
1. Launch ALL discovery subagents at once (up to ~6 per message). Each runs its queries via the `browserless_search` tool (request ~25 results per query) and writes the results to `/tmp/company_discovery_batch_{N}.json`.
