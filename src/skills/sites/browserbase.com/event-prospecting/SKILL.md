...
license: MIT
compatibility: Requires the Browserless MCP server (endpoint + bearer token). The `browserless_search` tool covers web search; `browserless_agent` (goto + text/evaluate) covers JS-rendered speaker pages.
...
**Required**: the Browserless MCP server configured (endpoint + bearer token). Use `browserless_search` for web search and `browserless_agent` (`goto` + `text`/`evaluate`) for JS-heavy speaker pages.
...
- A person's `hook` MUST quote or paraphrase a specific finding from a `browserless_search` result (podcast title, blog headline, GitHub repo, talk abstract). If no public signal exists in the last 6 months, fall back to event-context (their talk title at this event).
...
# workflow.md (rule 3 of the ICP Triage prompt). Burning a real browserless_search to
...
- `{TOTAL}` → number of lines in this batch (substitute into `# tool call N/{TOTAL}`)
...
1. `browserless_search "{name} {company} linkedin"` (always)
2. `browserless_search "{name} podcast OR talk OR blog 2026"` (deep+)
3. `browserless_search "{name} github"` (deeper)
4. `browserless_search "{name} site:x.com OR site:twitter.com"` (deeper, best-effort)
...
- `{LANES}` → `2` for deep mode, `4` for deeper mode (substituted into `# tool call N/{LANES}`)
