...
compatibility: 'Requires Node 18+, the browse CLI (`npm install -g browse`) with a CDP connection, and optionally `jq` for ad-hoc querying of the bisected JSONL files. For remote Browserbase sessions, also requires `BROWSERBASE_API_KEY`. The skill scripts themselves use only the Node standard library — no `npm install` step.'
license: MIT
...
Verify a CDP connection exists:

`bash
browse --help | grep -q "^\s*cdp " || echo "a CDP connection not available — update browse"
`
...
1. **Firehose**: `a CDP connection <target>` streams every CDP event as one JSON object per line to `cdp/raw.ndjson`.
2. **Sampler**: a polling loop calls `a screenshot --path <file>` and a ` <target>` HTML-dump command on an interval (default 2s). The helper passes when it samples so it can attach to the traced target from its own process; once the session is attached to a CDP target, follow-up commands in that session do not need to repeat.
...
# 1. Launch Chrome with its remote debugging port open on 9222

# (any user-data-dir keeps it isolated).
...
# 3. Run your main automation against port 9222 — point your driver

# (Playwright / Stagehand / etc.) at the page over CDP on port 9222.
...
> Browserbase ends a session as soon as its last CDP client disconnects. **Create the session in keep-alive mode, then attach automation to the session's `connectUrl` before or together with the tracer.** `bb-capture.mjs` handles the keep-alive session and tracer setup; your automation still needs to attach.
...
node scripts/bb-capture.mjs my-run
...
# Fetch the session's connectUrl via the Browserbase platform API, then point

# your driver at it over CDP and navigate to the page(s) you want traced.

# 3. Stop the tracer, bisect, then pull platform artifacts and release.

node scripts/stop-capture.mjs my-run
node scripts/bisect-cdp.mjs my-run
node scripts/bb-finalize.mjs my-run
`

Attaching to a session that's _already running_ (e.g. one your production worker created) — `bb-capture.mjs` accepts a session id instead of:

`bash

# Pick a running session via the platform session-list API (filter client-side

# by status == "RUNNING" and read .id) — there is no server-side status filter.
...
node scripts/bb-finalize.mjs mid-flight-debug # without: leave the session running
`
...
- `<run>/browserbase/session.json` — final platform session-get snapshot (proxyBytes, status, ended_at, viewport, …)
- `<run>/browserbase/logs.json` — platform session-logs output. **Often empty.** The CDP firehose in `cdp/raw.ndjson` is the source of truth; this is a side channel.
...
`.o11y/<run-id>/
 ...
 session.json final platform session-get snapshot (proxyBytes, status, ended_at, …)
 logs.json platform session-logs output (often [])
 downloads.zip platform session-downloads output (only if the session downloaded files)`
...
1. **Use `bb-capture.mjs` on Browserbase**: it enforces keep-alive mode, fetches the connectUrl, captures the debugger URL, and stamps the manifest. Doing it manually invites mistakes.
2. **Don't a session you don't own**: `bb-finalize.mjs` is for sessions _you_ created with. When attaching to a production session via `bb-capture.mjs <session-id>`, run `bb-finalize.mjs` without so the original automation keeps running.
3. **Order matters for remote**: on Browserbase, attach the main automation client before (or together with) the tracer, and create the session in keep-alive mode. Otherwise the session ends as soon as the tracer's WS closes.
...
6. **Reuse one Browserbase session for the automation client on remote** by attaching your driver to that session's `connectUrl` over CDP (with a ` <name>` label). The flag names the local the session daemon; it is not a Browserbase session attach flag.
...
- **`a CDP connection exited immediately`**: usually means the target is unreachable (wrong port) or the Browserbase session has already ended. For remote, verify the session via the platform session-get API — if `status` is `COMPLETED`, recreate it in keep-alive mode and attach automation first.
...
- **Browserbase session ends mid-run**: it likely hit. Recreate with a higher timeout (`BB_SESSION_TIMEOUT=1800 node scripts/bb-capture.mjs ...`) or remove the timeout flag.
- **`bb-capture.mjs <id>` says "not RUNNING"**: the session you tried to attach to ended. List candidates via the platform session-list API, filtering client-side for `.status == "RUNNING"`, and try again.
- **`browserbase/logs.json` is empty `[]`**: expected — the platform session-logs endpoint is sparse in practice. The CDP firehose in `cdp/raw.ndjson` is the source of truth.
