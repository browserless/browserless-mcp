...
## Proxy (optional)
Proxy config is one **top-level \`proxy\` object**: \`{ "proxy": { "proxy": "residential", "proxyCountry": "us" } }\`. Its \`proxy\`, \`proxyCountry\`, \`proxyState\`, \`proxyCity\`, \`proxySticky\`, \`proxyLocaleMatch\`, \`proxyPreset\`, and \`externalProxyServer\` fields belong inside that object and are applied when the session opens. **NEVER call \`proxy\` as a method inside \`commands\`** — a \`{ method: "proxy", ... }\` JSON-RPC mutation does NOT change the upstream proxy on an already-open session and will silently no-op.
...
- \`proxy: "residential" | "datacenter"\` — choose a Browserless network; \`proxyCountry: "us"\` — geo (ISO-2); \`proxyState\` / \`proxyCity\` (paid plans, 401 otherwise); \`proxySticky: true\` — stable IP; \`proxyLocaleMatch: true\` — match locale; \`proxyPreset\` — residential-only named config; \`externalProxyServer: "http://u:p@host:port"\` — bring your own (http(s) only)
- Geo/sticky require \`proxy\` or \`externalProxyServer\`; \`proxyPreset\` requires \`proxy: "residential"\`
...
## Terminal-Goal Check
Before declaring done, restate the user's terminal deliverable in one line and verify your evidence *directly* supports it — not a sibling question.
**Empty-state substitution.** An empty/zero/null result from a resource that normally needs scope or filter context is evidence the *precondition* wasn't met — not evidence the question is answered (e.g. zero results behind an unmet filter). Fix the precondition; don't return the empty result as the answer.
**Multi-step preconditions.** When the task names multiple steps ("go to X, then Y, report Z"), assess preconditions for the *full chain* before treating any step as optional. A blocker on step N blocks the whole task even if step 1 returned data.

## Skills (auto-injected)
SKILL blocks auto-inject between \`--- SKILL: <id> ---\` markers when the page/error needs special handling. Read carefully.
Load manually via **browserless_skill** if suspected but not injected:
- \`shadow-dom\` — deep selectors, iframe targeting
- \`cookie-consent\` — vendor-specific dismiss recipes
- \`modals\` — closing dialogs and alertdialogs
...
## Iframes
Snapshots include a \`Frames\` list (cross-origin iframes) when present. Elements inside a frame are tagged \`[frame#N]\` and carry a \`deep-ref=< *url* css\` selector that already pierces the frame — pass it as-is to \`click\`/\`type\`/\`hover\`/\`checkbox\`. No frame switching needed. \`shadow-dom\` skill auto-loads when frames present.
...
1. Check in-memory snapshot (text/values already there)
2. **text** { selector } — from specific element
3. **html** { selector } — raw HTML

## Batching — Maximize Per Call
Plan ALL actions from snapshot before next snapshot.

**Process:**
1. Classify actions: **safe** (type, hover, scroll, select, checkbox) vs. **page-changing** (click, goto)
2. Batch: safe FIRST → page-changing LAST
3. For forms: if submit button is in snapshot, batch type + click in one call
4. Don't batch across navigations
...
## Async
After async triggers (search, submit), use \`wait*\` before snapshot — \`waitForResponse\` best when the API URL is known. \`dynamic-content\` skill auto-loads on timeout.

## Error Recovery
Errors tagged \`Category: <NAME>\`:
- **SELECTOR_MISS** — re-snapshot; retry \`< selector\` if not already deep-ref
- **SESSION_LOST** — a fresh session was opened automatically; re-goto + snapshot (prior state gone)
- **UNAUTHORIZED** / **FORBIDDEN** — pick a different path
- **NOT_FOUND** — different URL
- **SERVER_ERROR** — backoff, retry once
- **NAVIGATION_FAILED** — verify URL
- **TIMEOUT** — longer wait or different signal
...
\`! NOTICE: URL changed cross-origin\` = prior plan/refs invalid, re-plan.
Never retry the same failed action without re-snapshot.
